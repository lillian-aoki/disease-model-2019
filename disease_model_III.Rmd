...
Spring Warming was derived from MUR SST product - 1 km pixels. Because many of our sites are in small estuaries, the coastal masking of land excludes many sites (11 of 32 meadows). Here, I've added in two more sites - OR-A and OR-B, in Coos Bay, where there is a NERR water quality sampling station. The NERR water temps fall between OR-A and OR-B for late summer and fall, so I've used the NERR data as the temperature data for both OR-A and OR-B. This brings the total of sites included in the model to 23.
...
```{r data,echo=FALSE}
sp <- read.csv("input data/spring_warming_shoots_supp.csv")
...
I am a bit hesitant about the model, partly because the BB sites aren't in it, and also because other temperature variables likely won't show the same regional split. But for now it is interesting.
