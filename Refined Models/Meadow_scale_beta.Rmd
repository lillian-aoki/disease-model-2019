...
fit_sev3 <- betareg(SeverityMean~CPTempAnomaly,
                     ...
                     weights = Count,
                     link = "logit")
fit_sev4 <- betareg(SeverityMean~ CPTempAnomaly + BladeAreaMean + DensityShootsMean,
                     data=dat,
                     weights = Count,
                     link = "logit")
...
fit_sev6 <- betareg(SeverityMean~ DensityShootsMean,
                     ...
                     weights = Count,
                     link = "logit")
AIC(fit_sev1,fit_sev2,fit_sev3,fit_sev4,fit_sev5,fit_sev6)
```
...
legend <- get_legend(a1+theme(legend.box.margin = margin(6,0,0,0),
                              legend.direction = "horizontal"))
pcombo <- plot_grid(a1a,b1,nrow=2, labels=c("A","B"))
scombo <- plot_grid(d1,c1,nrow=2, labels=c("C","D"))
total <- plot_grid(pcombo,scombo,ncol=2)
